...
(id INTEGER PRIMARY KEY, name TEXT, aisle INTEGER, price INTEGER, quantity INTEGER);
...
SELECT  AVG(price) AS averageprice , quantity  FROM storefront 
GROUP BY price,quantity 
ORDER BY price DESC;

SELECT * FROM storefront
WHERE price > (SELECT AVG(price) FROM storefront);
